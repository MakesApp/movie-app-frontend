...
import { useGetLatestMoviesQuery, useGetMovieBySearchTermQuery, useGetTopMoviesQuery } from '../../services/api/movieSlice';
...
console.log(filteredMovies);
  useEffect(() => {
    ...
    else setFilteredMovies(mergedmovies.filter(mov => mov.name.toLowerCase().includes(searchQuery.toLowerCase())));
  }, [searchQuery]);
...
path={`/search/${searchQuery}/page`}
    />
    <MovieList movies={filteredMovies.slice((page-1)*MOVIES_PER_PAGE, page*MOVIES_PER_PAGE-1)}/>
    </>
  );
};
